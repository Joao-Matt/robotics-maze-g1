        ...
        rows.append(
            {
                ...
                "localization_final_error_per_meter": localization.get("final_position_error_per_meter") if isinstance(localization, dict) else None,
                "localization_yaw_p95_deg": localization.get("yaw_p95_deg") if isinstance(localization, dict) else None,
                "localization_drift_scale": localization.get("distance_scale") if isinstance(localization, dict) else None,
                "localization_translation_jumps": localization.get("sudden_translation_jump_count") if isinstance(localization, dict) else None,
                "localization_yaw_jumps": localization.get("sudden_yaw_jump_count") if isinstance(localization, dict) else None,
...
def aggregate_kpis(report: dict[str, Any], compare: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "localization": {
            ...
            "final_error_per_meter_median": median(values_for(rows, "localization_final_error_per_meter")),
            "yaw_p95_median_deg": median(values_for(rows, "localization_yaw_p95_deg")),
            "drift_scale_median": median(values_for(rows, "localization_drift_scale")),
            "translation_jumps_total": sum(numeric(row.get("localization_translation_jumps")) or 0.0 for row in rows),
            "yaw_jumps_total": sum(numeric(row.get("localization_yaw_jumps")) or 0.0 for row in rows),
        },
        "mapping": {
...
def render_html_report(report: dict[str, Any], compare: dict[str, Any] | None = None) -> str:
    ...
    blocks = [
        ...
        metric_block("Localization", "Did it know where it was?", [
            ...
            ("Final drift / meter median", localization["final_error_per_meter_median"], "x"),
            ("Yaw p95 median", localization["yaw_p95_median_deg"], "deg"),
